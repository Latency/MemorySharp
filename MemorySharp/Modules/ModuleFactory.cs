...
public class ModuleFactory : IFactory
{
    ...
    /// </summary>
    public RemoteModule MainModule { get; init; }
    ...
    internal ModuleFactory(MemorySharp memorySharp)
    {
        ...
        MainModule = FetchModule(MemorySharp.Process.MainModule ?? throw new ArgumentNullException());
    }
    ...
    public void Eject(string moduleName)
    {
        ...
        var module = RemoteModules.FirstOrDefault(m => m.ModuleName == moduleName);
    ...
    ///     A module name (not case sensitive). If the file name extension is omitted, the default library extension .dll is appended.
    /// </param>
    /// <returns>A new instance of a <see cref="RemoteModule" /> class.</returns>
    protected async Task<RemoteModule> FetchModule(string moduleName)
    {
        ...
        return mod is null ? throw new NullReferenceException() : new RemoteModule(mod, MemorySharp.Modules)
        {
            Region = new RemoteRegion(MemorySharp, mod.BaseAddress)
        };
    ...
    /// </summary>
    /// <param name="path">
    ///     The path of the module. This can be either a library module (a .dll file) or an executable module (an .exe file).
    /// </param>
