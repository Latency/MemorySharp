...
public static class ModuleCore
{
    ...
    public static ProcessModule LoadLibrary(string libraryPath)
    {
        ...
        // Load the library
        return NativeMethods.LoadLibrary(libraryPath) != IntPtr.Zero
                   ? Process.GetCurrentProcess().Modules.Cast<ProcessModule>().First(m => m.FileName == libraryPath)
    ...
    public static IntPtr GetProcAddress(string moduleName, string functionName)
    {
        ...
        if (module == null)
            throw new ArgumentException($"Couldn't get the module {moduleName} because it doesn't exist in the current process.");

        // Get the function address
        var ret = NativeMethods.GetProcAddress(module.BaseAddress, functionName);

        // Check whether the function was found
        return ret != IntPtr.Zero ? ret : throw new Win32Exception($"Couldn't get the function address of {functionName}.");
    }
    ...
    public static IntPtr GetProcAddress(ProcessModule module, string functionName) => GetProcAddress(module.ModuleName, functionName);
