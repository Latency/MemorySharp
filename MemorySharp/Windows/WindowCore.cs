...
public static class WindowCore
{
    ...
    /// <summary>
    ///     Activates the specified window in a more robust way than the default methods. Due to the security adjustments to
    ///     the SetForegroundWindow API as of Windows Vista (2007).
    ///     ‘An application can’t force another application to be in the foreground, unless it created it or related to it
    ///     somehow’ (the full rules can be found here:
    ///     http://msdn.microsoft.com/en-us/library/windows/desktop/ms633539(v=vs.85).aspx, see ‘Rules’).
    /// </summary>
    /// <param name="windowHandle">A handle to the window.</param>
    public static void ActivateWindow(IntPtr windowHandle)
    {
        var appThreadId        = NativeMethods.GetCurrentThreadId();
    ...
    ///     <seealso cref="ForceWindowToForeground(IntPtr)" /> or <seealso cref="SetFocusAttached(IntPtr)" /> for an example
    ///     usage.
    /// </summary>
    ...
    public static void AttachedThreadInputAction(Action action)
    {
        ...
        try
        {
            threadsAttached = foregroundThreadId == appThreadId || NativeMethods.AttachThreadInput(foregroundThreadId, appThreadId, true);
    ...
    public static void ForceWindowToForeground(IntPtr hwnd)
    {
        AttachedThreadInputAction(() =>
        {
            ...
            _ = NativeMethods.ShowWindow(hwnd, WindowStates.Show);
        });
    }

    /// <summary>
    ///     By using the AttachThreadInput function, a thread can attach its input processing to another thread. This allows a
    ///     thread to call SetFocus to set the keyboard focus to a window attached to another thread's message queue. This is
    ///     done here.
    /// </summary>
