// ****************************************************************************
// Project:  MemorySharp
// File:     MicrosoftX64CallingConvention.cs
...
///     The remaining parameter are pushed on the stack in the right-to-left order. The caller cleans the arguments from
///     the stack.
...
public class MicrosoftX64CallingConvention : ICallingConvention
{
    ...
    public void FormatCall(IntPtr function, IntPtr[] parameters, List<string> instructions)
    {
        ...
        instructions.AddRange(parameters.Skip(4).Reverse().Select(parameter => $"push {parameter}"));
