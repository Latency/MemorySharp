// ****************************************************************************
// Project:  MemorySharp
// File:     FastcallCallingConvention.cs
...
///     The remaining parameter are pushed on the stack in the right-to-left order. The callee cleans the arguments from
///     the stack.
...
public class FastcallCallingConvention : ICallingConvention
{
    ...
    public void FormatCall(IntPtr function, IntPtr[] parameters, List<string> instructions)
    {
        ...
        instructions.AddRange(parameters.Skip(2).Reverse().Select(parameter => $"push {parameter}"));
