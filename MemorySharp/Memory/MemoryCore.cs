...
public static class MemoryCore
{
    ...
    public static IntPtr Allocate(SafeMemoryHandle processHandle, long size, MemoryProtectionFlags protectionFlags = MemoryProtectionFlags.ExecuteReadWrite, MemoryAllocationFlags allocationFlags = MemoryAllocationFlags.Commit)
    {
        ...
        return ret != IntPtr.Zero ? ret : throw new Win32Exception($"Couldn't allocate memory of {size} byte(s).");
    }
    ...
    public static SafeMemoryHandle OpenProcess(ProcessAccessFlags accessFlags, int processId)
    {
        ...
        return handle is { IsInvalid: false, IsClosed: false } ? handle : throw new Win32Exception($"Couldn't open the process {processId}.");
    }
    ...
    /// <param name="size">The number of bytes to be read from the specified process.</param>n
    ...
    public static MemoryProtectionFlags ChangeProtection(SafeMemoryHandle processHandle, IntPtr address, IntPtr size, MemoryProtectionFlags protection)
    {
        ...
        return NativeMethods.VirtualProtectEx(processHandle, address, size, protection, out var oldProtection)
                   ? oldProtection
                   : throw new Win32Exception($"Couldn't change the protection of the memory at 0x{address.ToString("X")} of {size} byte(s) to {protection}.");
    }
    ...
    public static unsafe ProcessBasicInformation NtQueryInformationProcess(SafeMemoryHandle processHandle)
    {
        ...
        var ret = NativeMethods.NtQueryInformationProcess(processHandle, (int)ProcessInformationClass.ProcessBasicInformation, infoPtr, Unsafe.SizeOf<ProcessBasicInformation>(), out _);

        // If the function succeeded
        return ret == 0 ? info : throw new ApplicationException($"The process information cannot be queried; error code '{ret}'.");
    }
    ...
    public static MemoryBasicInformation Query(SafeMemoryHandle processHandle, IntPtr baseAddress)
    {
        ...
        if (NativeMethods.VirtualQueryEx(processHandle, baseAddress, out var memoryInfo, MarshalType<MemoryBasicInformation>.SizeAsPointer) != 0)
            return memoryInfo;
    ...
    public static IEnumerable<MemoryBasicInformation> Query(SafeMemoryHandle processHandle, IntPtr addressFrom, IntPtr addressTo)
    {
        ...
        do
        {
            ...
            ret = NativeMethods.VirtualQueryEx(processHandle, addressFrom, out var memoryInfo, MarshalType<MemoryBasicInformation>.SizeAsPointer);
