...
/// <summary>
///     Provides an allocator that is able to control the alignment of a structure in memory, such as DECLSPEC_ALIGN in
///     C/C++.
...
/// </summary>
/// <remarks>
///     Internally, a chunk of memory is allocated on the stack with the size of the alignment plus the size og the
///     structure.
///     The alignment is then computed using a modulus and a pointer on the aligned address is reinterpreted as the
///     structure.
/// </remarks>
public static class StackAllocAlignment
{
    /// <summary>
    ///     A delegate that behaves similarly to <see cref="Action{T}" /> and requires that the type parameter is an unmanaged
    ///     type.
    ...
    public delegate void RefAction<T>(ref T unmanagedStruct)
        where T : unmanaged;
    ...
    /// <remarks>
    ///     The structure can only be used in the callback. Any usage of it outside of that scope manipulate a freed
    ///     memory.
    /// </remarks>
