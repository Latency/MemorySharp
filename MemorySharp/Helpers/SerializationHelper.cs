...
public static class SerializationHelper
{
    ...
    public static string ExportToJsonString<T>(T obj) => JsonSerializer.Serialize(obj);

    #endregion

    #region ImportFromJsonString
    ...
    /// <param name="serializedObj">The string representing the serialized object.</param>
    /// <returns>The deserialized object.</returns>
    public static T ImportFromJsonString<T>(string serializedObj) => JsonSerializer.Deserialize<T>(serializedObj);
    ...
    /// <param name="path">The path where the file is saved.</param>
    /// <param name="encoding">The encoding to generate.</param>
    public static void ExportToJsonFile<T>(T obj, string path, Encoding encoding)
    {
        // Create the stream to write into the specified file
        using var file = new StreamWriter(path, false, encoding);
        // Write the content by calling the method to serialize the object
        file.Write(ExportToJsonString(obj));
    }
    ...
    /// <param name="obj">The object to serialize.</param>
    /// <param name="path">The path where the file is saved.</param>
    public static void ExportToJsonFile<T>(T obj, string path) => ExportToJsonFile(obj, path, Encoding.UTF8);
    ...
    /// <param name="encoding">The character encoding to use. </param>
    /// <returns>The deserialized object.</returns>
    public static T ImportFromJsonFile<T>(string path, Encoding encoding)
    {
        // Create the stream to read the specified file
        using var file = new StreamReader(path, encoding);
        // Read the content of the file and call the method to deserialize the object
        return ImportFromJsonString<T>(file.ReadToEnd());
    }
    ...
    /// <param name="path">The path where the object is read.</param>
    /// <returns>The deserialized object.</returns>
    public static T ImportFromJsonFile<T>(string path) => ImportFromJsonFile<T>(path, Encoding.UTF8);
