...
using MemorySharp.Threading.Windows;
...
public class RemoteThread : IDisposable, IEquatable<RemoteThread>
{
        ...
        where TContext : unmanaged
    {
        ...
        if (!IsAlive)
            throw new ThreadStateException($"The context cannot be retrieved because the thread #{Id} is terminated.");
    ...
    public T GetExitCode<T>()
    {
        ...
        return ret.HasValue ? MarshalType<T>.PtrToObject(MemorySharp, ret.Value) : default;
    }
    ...
    public IntPtr GetRealSegmentAddress(SegmentRegisters segment, ref ThreadContext32 context)
    {
        ...
        LdtEntry entry = segment switch
        {
            ...
            _                   => throw new InvalidEnumArgumentException(nameof(segment), (int)segment, typeof(SegmentRegisters))
        };
