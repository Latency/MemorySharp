...
public static class ThreadCore
{
    ...
    /// <param name="context">
    ///     A pointer to a <see cref="ThreadContext32" /> structure that contains the context to be set in
    ...
    public static unsafe ThreadBasicInformation NtQueryInformationThread(SafeMemoryHandle threadHandle)
    {
        ...
        var   ret     = NativeMethods.NtQueryInformationThread(threadHandle, ThreadInformationClass.ThreadBasicInformation, infoPtr, MarshalType<ThreadBasicInformation>.SizeAsPointer, out var returnLength);
    ...
    public static unsafe ulong NtQueryInformationThread(SafeMemoryHandle threadHandle, ThreadInformationClass threadInformationClass)
    {
        ...
        // Get the thread info
        ulong info = 0;
        var   ret  = NativeMethods.NtQueryInformationThread(threadHandle, ThreadInformationClass.ThreadBasicInformation, &info, new IntPtr(sizeof(ulong)), out var returnLength);
