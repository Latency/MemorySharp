...
namespace MemorySharp.Threading.Windows;
...
public class ManagedTeb : RemotePointer
{
    ...
    internal ManagedTeb(MemorySharp memorySharp, RemoteThread thread) : base(memorySharp, FindTeb(thread.Handle)) => _offsets = memorySharp.Is64Process ? Singleton<Teb64Offsets>.Instance : Singleton<Teb32Offsets>.Instance;
    ...
    private static IntPtr FindTeb(SafeMemoryHandle threadHandle) => ThreadCore.NtQueryInformationThread(threadHandle).TebBaseAddress;
