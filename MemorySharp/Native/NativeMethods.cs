...
public static class NativeMethods
{
    ...
    ///     retrieved.
    ///     The structure structure is highly processor specific and because of that, that parameter is a pointer on the
    ///     correct structure.
    ...
    ///     If the function fails, the return value is zero. To get extended error information, call
    ///     <see cref="Marshal.GetLastWin32Error" />.
    ///     The return type is initially a BOOL (of 4 bytes length), which corresponds to an integer to prevent any
    ///     marshalling.
    /// </returns>
    [DllImport("kernel32.dll", SetLastError = true)]
    ...
    /// </summary>
    /// <param name="handle">
    ///     A handle to the process. The handle must have the <see cref="ProcessAccessFlags.QueryInformation" /> or
    ///     <see cref="ProcessAccessFlags.QueryLimitedInformation" /> access right.
    /// </param>
    /// <param name="isWow64Process">
    ///     A pointer to a value that is set to TRUE if the process is running under WOW64 on an Intel64 or x64 processor. If
    ///     the process is running under 32-bit Windows, the value is set to FALSE.
    ///     If the process is a 32-bit application running under 64-bit Windows 10 on ARM, the value is set to FALSE. If the
    ...
    public static extern unsafe uint NtQueryInformationProcess(SafeMemoryHandle processHandle, ProcessInformationClass processInformationClass, void* processInformation, IntPtr processInformationLength, out IntPtr returnLength);
    ...
    ///     The class of information to retrieve and store in the thread information buffer
    ///     parameter.
    /// </param>
    ...
    ///     successful,
    ///     this is the size of the information written to the buffer pointed to by the thread information parameter, but if
    ...
    [DllImport("ntdll.dll", SetLastError = true)]
    public static extern unsafe uint NtQueryInformationThread(SafeMemoryHandle handle, ThreadInformationClass threadInformationClass, void* threadInformation, IntPtr threadInformationLength, out IntPtr returnLength);
    ...
    ///     The identifier of the thread to which idAttach will be attached. This thread cannot be a
    ///     system thread. <remarrks>>A thread cannot attach to itself.Therefore, idAttachTo cannot equal idAttach.</remarrks
    /// </param>
    /// <param name="fAttach">
    ...
    /// </returns>
    /// <remarks>
    ///     By using the AttachThreadInput function, a thread can share its input states (such as keyboard states and the
    ///     current focus window) with another thread. Keyboard and mouse events received by both threads are processed in the
    ///     order they were received until the threads are detached by calling AttachThreadInput a second time and specifying
    ///     FALSE for the fAttach parameter.
    ///     The AttachThreadInput function fails if either of the specified threads does not have a message queue.The system
    ///     creates a thread's message queue when the thread makes its first call to one of the USER or GDI functions. The
    ///     AttachThreadInput function also fails if a journal record hook is installed. Journal record hooks attach all input
    ///     queues together.
    ...
    ///     value is NULL. To get extended error information, call GetLastError function. Extended error
    ///     ERROR_INVALID_PARAMETER(0x57) means that window is in disabled state.
    /// </returns>
    /// <remarks>
    [DllImport("user32.dll", SetLastError = true)]
