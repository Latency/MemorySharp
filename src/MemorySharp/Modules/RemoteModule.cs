...
public class RemoteModule : RemoteRegion
{
    ...
    public RemoteFunction FindFunction(string functionName)
    {
        ...
        var localModule      = Process.GetCurrentProcess().Modules.Cast<ProcessModule>().FirstOrDefault(m => string.Equals(m.FileName, Path, StringComparison.CurrentCultureIgnoreCase));
